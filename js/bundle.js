...
function cards() {
  // Use class for cards
  class MenuCard {
    constructor(src, alt, subtitle, desc, cost, parentSelector) {
      this.src = src;
      this.alt = alt;
...
function modal(modalSelector, triggerSelector, modalTimerId) {
  // Modal
  const modal = document.querySelector(modalSelector),
        buttonsOpen = document.querySelectorAll(triggerSelector);
  buttonsOpen.forEach(btn => {
    btn.addEventListener("click", () => openModal(modalSelector, modalTimerId));
  });
  modal.addEventListener("click", event => {
    if (event.target === modal || event.target.getAttribute("data-closed") == "") {
...
__webpack_require__.r(__webpack_exports__);
function tabs(tabsParentSelector, tabsButtonsSelector, tabsContentSelector, activeClass) {
  //Tabs
